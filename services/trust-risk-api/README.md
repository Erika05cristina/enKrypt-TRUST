...
**LLM (Ollama):** con `TRUST_LLM_ENABLED=true` y Ollama en marcha, la respuesta `200` incluye `llmAnalysis` con **JSON estructurado** (`verdict`, `flags`, `summary`, `bullets`, más `text` legible). El prompt **standard** pide un barrido corto (≤3 bullets, resumen de una frase, `technical_rationale` vacío); el tier **deep** pide más bullets, resumen extendido y `technical_rationale` obligatorio. Ambos tiers pueden devolver `ai_risk_score` (0–100, más alto = más riesgo): si viene informado, el API **mezcla** ese valor con `paidRiskScore` / `reputationScore` del motor (peso ~12% standard, ~65% deep) para que el premium se note en las puntuaciones. Calldata / bytecode / Solidity solo se truncan si superan los límites de env. Si Ollama falla, el `200` sigue con B3 y `llmSkippedReason`. La IA **no** sustituye auditoría humana (ver `disclaimer` en `llmAnalysis`).
